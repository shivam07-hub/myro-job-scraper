...
| Coforge | https://careers.coforge.com | careers.coforge.com | MTUxNzM= | https://public.zwayam.com/jobs/search | 60 | ✅ CRACKED 2026-08-21 — Scrapling revealed the Zwayam route; promoted to the existing direct provider; full no-publish probe returned 60 non-empty source JDs without Firecrawl |
...
| S&P Global | https://careers.spglobal.com/jobs | Jibe public jobs API | `GET https://careers.spglobal.com/api/jobs?location=India&page=N` | `location=India` plus Python `is_india()` on `full_location` | 61 | ✅ CRACKED 2026-08-21 — Scrapling route discovery promoted to direct paginated Jibe API; full no-publish probe returned 61 India jobs with non-empty JDs and Workday apply URLs |
| Elevation Capital | https://apply.workable.com/elevation-capital-3/ | Workable | `POST https://apply.workable.com/api/v3/accounts/elevation-capital-3/jobs` + `GET /api/v2/accounts/elevation-capital-3/jobs/{shortcode}` | `location.countryCode == IN` | 10 India / 22 global | ✅ CRACKED 2026-08-21 — direct public Workable listing and detail APIs; full no-publish probe returned 10 India jobs; fail-closed if any selected detail JD cannot be fetched |
| Yubi | https://go-yubi.com/careers | Yubi HTML + Zoho Recruit details | `GET https://go-yubi.com/careers` for official links + per-job Zoho `/jobs/Careers/{id}/{slug}` | official Yubi page is the listing source of truth; detail `Country == India` | 19 live / 22 linked | ✅ CRACKED 2026-08-21 — browser-like direct HTML returns current links; per-job embedded Zoho JSON provides full JDs; 3 withdrawn links currently return Zoho's generic inactive page and are omitted; hidden Zoho listing payload is deliberately ignored because it contains historical rows |
...
| Amdocs | https://jobs.amdocs.com/careers?domain=amdocs.com&location=India | https://jobs.amdocs.com | amdocs.com | 1+ validated | ✅ CRACKED 2026-08-21 — Scrapling exposed the PCSX route; promoted to the existing direct PCSX provider with JSON-LD detail JDs |
